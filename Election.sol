...
contract Election is Ownable {
    ...
    // Modificateur pour le Président (non utilisé pour la création de résolution maintenant)
    modifier onlyPresident() {
        require(msg.sender == president, "Vous devez etre le president de seance pour cette action.");
        _;
    }

    // Modificateur pour le Scrutateur
    modifier onlyScrutateur() {
        require(msg.sender == scrutateur, "Vous devez etre le scrutateur pour cette action.");
        _;
    }

    // Nouveau modificateur pour le Secrétaire (Owner)
    modifier onlySecretaire() {
        require(msg.sender == secretaire, "Vous devez etre le secretaire pour cette action.");
        _;
    }
    ...
    // Ajouter une résolution (uniquement par le Secrétaire / Owner)
    function addResolution(string memory _title) public onlySecretaire {
        resolutionsCount++;
